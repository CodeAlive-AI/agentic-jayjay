...
- The left graph shows jj changes as a DAG with lanes for forks, merges, bookmarks, tags, conflicts, divergent changes, and working-copy state. Each row shows bookmark and tag chips, the author avatar, a relative timestamp, green/red `+N`/`-M` line counts for source files (linguist programming/markup/prose), gray parenthetical totals when other files also changed, and the shortest unique change-id prefix highlighted.
