...
fn action_row(
    ...
    let row = div()
        .id(SharedString::from(format!("app-menu-item-{name}")))
        .debug_selector({
            let name = name.clone();
        ...
        .child(checkmark(checked, icon_color))
...
fn checkmark(checked: bool, color: u32) -> AnyElement {
    div()
        .flex_none()
        .w(px(14.))
        .child(if checked {
            icons::icon(glyph::CHECK, 12., color)
        } else {
            div()
        })
        .into_any_element()
