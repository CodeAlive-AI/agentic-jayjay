...
use super::model::{active_bookmark_sync_label, truncate_middle, working_copy_stat_label};
...
{
    status_item_base(id, Some(glyph_str), text, t)
        .cursor_pointer()
...
fn pr_link(pr: &PrInfo, t: &Theme, cx: &mut Context<RepoWindow>) -> AnyElement {
    let glyph_str = match pr.checks {
        ChecksStatus::Passing => glyph::CHECK,
        ChecksStatus::Failing => glyph::X,
        ChecksStatus::Pending | ChecksStatus::None => glyph::DOT,
    };
