use gpui::{
    ClipboardItem, InteractiveElement, IntoElement, ParentElement, SharedString,
...
pub(super) fn command_row(command: &'static str, t: &Theme) -> impl IntoElement {
    div()
        ...
        .child(
            div()
                .id(SharedString::from(format!("onboarding-copy-{command}")))
                .flex()
                .items_center()
                .justify_center()
                .w(px(24.))
                ...
                .child(icons::icon(glyph::COPY, 13., t.fg_dim)),
        )
}
